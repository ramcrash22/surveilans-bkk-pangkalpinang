...
const tujuanKeberangkatanOptions = ["Dalam Negeri", "Luar Negeri"];
...
export default function KapalLautKeberangkatanForm({
  formData,
  onChange,
  ...
  const handleInternalSubmit = async (e) => {
    ...
    try {
      ...
      const API_URL = "http://127.0.0.1:8000/api/kapal-laut/keberangkatan"; // <--- GANTI DENGAN URL API ASLI ANDA
      // Contoh: "http://localhost:8000/api/kapal-laut/keberangkatan" atau "https://namaaplikasianda.com/api/kapal-laut/keberangkatan"
  ...
  return (
    <div className={styles.formContainer}>
      <div className={styles.formTitle}>Form Keberangkatan Kapal Laut</div>
      ...
      <form onSubmit={handleInternalSubmit} autoComplete="off">
        ...
        <label className={styles.formLabel}>
          Pelabuhan Keberangkatan<span className={styles.required}>*</span>
          <input
            ref={inputRefs.pelabuhanKeberangkatan}
            className={styles.formInput}
            type="text"
            name="pelabuhanKeberangkatan"
            value={formData.pelabuhanKeberangkatan || ""}
            onChange={handleChange}
            placeholder="Masukkan pelabuhan keberangkatan"
          />
          {renderError("pelabuhanKeberangkatan")}
        </label>
        <label className={styles.formLabel}>
          Negara/Kabupaten/Kota Keberangkatan<span className={styles.required}>*</span>
          <input
            ref={inputRefs.negaraKabupatenKotaKeberangkatan}
            className={styles.formInput}
            type="text"
            name="negaraKabupatenKotaKeberangkatan"
            value={formData.negaraKabupatenKotaKeberangkatan || ""}
            onChange={handleChange}
            placeholder="Masukkan negara/kabupaten/kota keberangkatan"
          />
          {renderError("negaraKabupatenKotaKeberangkatan")}
        </label>
        <label className={styles.formLabel}>
          Tujuan Keberangkatan<span className={styles.required}>*</span>
          <select
            ref={inputRefs.tujuanKeberangkatan}
            className={styles.formSelect}
            name="tujuanKeberangkatan"
            value={formData.tujuanKeberangkatan || ""}
            onChange={handleChange}
          >
            <option value="">Pilih tujuan</option>
            {tujuanKeberangkatanOptions.map((w) => (
              <option key={w} value={w}>{w}</option>
            ))}
          </select>
          {renderError("tujuanKeberangkatan")}
        </label>
        <label className={styles.formLabel}>
          Status Kes. Pelabuhan/Negara Tujuan<span className={styles.required}>*</span>
          <select
