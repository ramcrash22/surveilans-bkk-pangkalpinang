...
export default function PesawatKedatanganForm({
  ...
  const validate = () => {
    ...
    if (!formData.bandaraAsal) newErrors.bandaraAsal = 'Bandara Asal harus diisi';
    if (
  ...
  return (
    <form onSubmit={handleSubmit} autoComplete="off" className={styles.formContainer}>
      ...
      <div>
        <label className={styles.formLabel}>
          Bandara Asal <span className={styles.required}>*</span>
        </label>
        <input
          type="text"
          className={styles.formInput}
          ref={inputRefs.bandaraAsal}
          value={formData.bandaraAsal || ''}
          onChange={(e) => handleChange('bandaraAsal', e.target.value)}
          placeholder="Your answer"
        />
        {errors.bandaraAsal && <div className={styles.errorText}>{errors.bandaraAsal}</div>}
      </div>
